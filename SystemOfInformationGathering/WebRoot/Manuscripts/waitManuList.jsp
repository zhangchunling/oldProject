...
<head>
	...
	$(function () {
	 ...
	 $(document).ready(function(){
    	...
    	var h="Manu_queryWaitList.action?flag=111&mu.currentPage=";
    	var str="";
    ...
    function exportExcel(){
			...
			if(ids.length>0){
				if(confirm("确认导出选中的约稿?")){
					window.location.href="Manu_exportExcl.action?exportType=wait&exportExcelType="+ids;
				}
			}else{
				if(confirm("请勾选要导出的稿件,[确定] 将导出所有约稿！")){
					window.location.href="Manu_exportExcl.action?exportType=wait&exportExcelType=all";
				}
			}
...
<form action="<%=basePath %>Manu_queryWaitList.action?mu.currentPage=1" method="post" class="definewidth m20">
...
<table class="table table-bordered table-hover definewidth m10" >
   <thead>
    ...
    <tr>
    	...
    	<tr>
    		...
    		<a class="btn btn-primary" href="<%=basePath %>Manu_queryWaitPageFile.action?pageFile.pf_mu_id=${m.mid}">查看提交稿件</a>
    		<input id="listSize" type="hidden" value="${m.mid }">
    		</td>
    	</tr>
    	</c:forEach>
  ...
  <table class="table table-bordered table-hover definewidth m10" >
          ...
          <a id="firstPage" href='<%=basePath %>Manu_queryWaitList.action?mu.currentPage=1&flag=111' >首页</a> 
          <a id="upPage" href='<%=basePath %>Manu_queryWaitList.action?mu.currentPage=${mu.currentPage<=1?mu.currentPage:mu.currentPage-1}&flag=111'>上一页</a>    
       	 <span id="atag">
       	 </span> 
           ...
           <a id="nextPage" href='<%=basePath %>Manu_queryWaitList.action?mu.currentPage=${mu.currentPage>=mu.totalPage?mu.currentPage:mu.currentPage+1}&flag=111' >下一页</a>
           <a id="lastPage" href='<%=basePath %>Manu_queryWaitList.action?mu.currentPage=${mu.totalPage}&flag=111' >尾页</a>
