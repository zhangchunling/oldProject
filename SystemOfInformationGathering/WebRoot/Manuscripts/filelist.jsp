    ...
    function exportExcel(){
			...
			if(ids.length>0){
				if(confirm("确认导出选中的稿件信息?")){
					window.location.href="PageFile_exportExcel?exportExcelType="+ids;
				}
			}else{
				if(confirm("请勾选要导出的稿件,[确定] 将导出所有稿件信息！")){
					window.location.href="PageFile_exportExcel?exportExcelType=all";
				}
			}
    ...
    //置空条件查询
     $(function () {
		...
		function delAll(){
			...
			if(ids.length>0){
				if(confirm("确认操作?")){
					window.location.href="PageFile_deletePfById?ids="+ids;
				}
			}else{
				alert("请选择要删除的稿件。");
			}
...
<body>
<form action="<%=basePath %>Manu_queryWaitPageFile.action?pageFile.currentPage=1" method="post" class="definewidth m20">
...
<table class="table table-bordered table-hover definewidth m10" >
   ...
   <tr >
    ...
    <!--遍历pf list  -->
    	...
    	<tr>
            ...
            <a href="<%=basePath %>Manu_queryPageFileById.action?pageFile.pf_id=${pf.pf_id}">查看详情</a><%-- --<a href="<%=basePath%>PageFile_deletePfById?pageFile.pf_id=${pf.pf_id}">删除</a> --%>
  ...
  <table class="table table-bordered table-hover definewidth m10" >
  	<tr><th colspan="5">  <div class="inline pull-right page">
          <a id="firstPage" href='<%=basePath %>Manu_queryWaitPageFile.action?pageFile.currentPage=1&flag=222' >首页</a> 
          <a id="upPage" href='<%=basePath %>Manu_queryWaitPageFile.action?pageFile.currentPage=${pageFile.currentPage<=1?pageFile.currentPage:pageFile.currentPage-1}&flag=222'>上一页</a>    
       	 <span id="atag">
       	 </span> 
           <!--&queryType=null是为了清空action里按条件查询留下的记录  -->
         	<%--  <a id="upPage" href='<%=basePath %>Manu_queryPf?pageFile.currentPage=${pageFile.currentPage<=1?pageFile.currentPage:pageFile.currentPage-1}'>上一页</a>    
           <a id="firstPage" href='<%=basePath %>Manu_queryPf?pageFile.currentPage=1' >首页</a> --%>
           <a id="nextPage" href='<%=basePath %>Manu_queryWaitPageFile.action?pageFile.currentPage=${pageFile.currentPage>=pageFile.totalPage?pageFile.currentPage:pageFile.currentPage+1}&flag=222' >下一页</a>
           <a id="lastPage" href='<%=basePath %>Manu_queryWaitPageFile.action?pageFile.currentPage=${pageFile.totalPage}&flag=222' >尾页</a>
