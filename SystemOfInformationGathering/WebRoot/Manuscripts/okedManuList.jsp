...
<head>
	...
	$(function () {
	 ...
	 $(document).ready(function(){
    	...
    	var h="Manu_queryOkedList.action?flag=1112&mu.currentPage=";
    	var str="";
    ...
    function exportExcel(){
			...
			if(ids.length>0){
				if(confirm("确认导出选中的约稿?")){
					window.location.href="Manu_exportExcl.action?exportType=OK&exportExcelType="+ids;
				}
			}else{
				if(confirm("请勾选要导出的稿件,[确定] 将导出所有约稿！")){
					window.location.href="Manu_exportExcl.action?exportType=OK&exportExcelType=all";
				}
			}
...
<form action="<%=basePath %>Manu_queryOkedList.action?mu.currentPage=1" method="post" class="definewidth m20">
...
<table class="table table-bordered table-hover definewidth m10" >
   <thead>
    ...
    <tr>
    	...
    	<tr>
    		...
    		<a href="<%=basePath%>Manu_queryManuById.action?mu.mid=${m.mid}">查看详情</a>&nbsp;&nbsp;&nbsp;
    		<a class="btn btn-primary" href="<%=basePath%>Manu_queryOkedPageList.action?pageFile.pf_mu_id=${m.mid}">查看已审核稿件</a>
    		<input id="listSize" type="hidden" value="${m.mid }">
    		</td>
    	</tr>
    	</c:forEach>
  ...
  <table class="table table-bordered table-hover definewidth m10" >
          ...
          <a id="firstPage" href='<%=basePath %>Manu_queryOkedList.action?mu.currentPage=1&flag=hehee' >首页</a> 
          <a id="upPage" href='<%=basePath %>Manu_queryOkedList.action?mu.currentPage=${mu.currentPage<=1?mu.currentPage:mu.currentPage-1}&flag=hehee'>上一页</a>    
       	 <span id="atag">
       	 </span> 
           ...
           <a id="nextPage" href='<%=basePath %>Manu_queryOkedList.action?mu.currentPage=${mu.currentPage>=mu.totalPage?mu.currentPage:mu.currentPage+1}&flag=hehee' >下一页</a>
           <a id="lastPage" href='<%=basePath %>Manu_queryOkedList.action?mu.currentPage=${mu.totalPage}&flag=hehee' >尾页</a>
