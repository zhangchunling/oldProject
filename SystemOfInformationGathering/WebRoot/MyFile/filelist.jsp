<%@page import="com.zrgk.manu.bean.*"%>
  ...
  <head>
    ...
    function exportExcel(){
			...
			if(ids.length>0){
				...
				if(confirm("请勾选要导出的稿件,[确定] 将导出当页所有稿件信息！")){
...
<body>
<form action="<%=basePath %>PageFile_queryPf.action?pageFile.currentPage=1" method="post" class="definewidth m20">
<table class="table table-bordered table-hover definewidth m10">
    <tr>
        <td width="10%" class="tableleft">类别</td>
        <td><input type="text" name="pageFile.pf_type" value="${pageFile.pf_type }" id="wqtype"/></td>

        <td width="10%" class="tableleft">文体</td>
        <td><input type="text" name="pageFile.pf_style" value="${pageFile.pf_style }" id="wqstyle"/></td>

        <td width="10%" class="tableleft">标题</td>
        <td><input type="text" name="pageFile.pf_name" value="${pageFile.pf_name }" id="wqname"/></td>
    </tr>
    <tr>
		  <td colspan="6"><center>
...
<table class="table table-bordered table-hover definewidth m10" >
   ...
   <tr >
    ...
    <!--遍历pf list  -->
    	...
    	<tr>
            ...
            <td style="vertical-align:middle;">${pf.pf_state==0?"未提交":pf.pf_state==1?"未审核":pf.pf_state==2?"已提交[未审核]":pf.pf_state==3?"审核通过":pf.pf_state==8?"一次审核未通过":pf.pf_state==5?"已录用":"未录用"}</td>
            <td style="vertical-align:middle;">
           	<c:if test="${pf.pf_mu_id==null }">
           	<a href="<%=basePath %>PageFile_bindManuById.action?pageFile.pf_id=${pf.pf_id}">&nbsp;绑定约稿</a>
           </c:if>
           	<c:if test="${pf.pf_mu_id>0 }">
           	<img src="<%=basePath%>Images/ok.PNG">已绑定
           	</c:if>
            <a href="<%=basePath %>PageFile_queryById.action?pageFile.pf_id=${pf.pf_id}&pageFile.pf_mu_id=${pf.pf_mu_id}">&nbsp;查看详情</a><%-- --<a href="<%=basePath%>PageFile_deletePfById?pageFile.pf_id=${pf.pf_id}">删除</a> --%>
  ...
  <table class="table table-bordered table-hover definewidth m10" >
  	<tr><th colspan="5">  <div class="inline pull-right page">
          <a id="firstPage" href='<%=basePath %>PageFile_queryPf.action?pageFile.currentPage=1' >首页</a> 
          <a id="upPage" href='<%=basePath %>PageFile_queryPf.action?pageFile.currentPage=${pageFile.currentPage<=1?pageFile.currentPage:pageFile.currentPage-1}'>上一页</a>    
       	 <span id="atag">
       	 </span> 
           ...
           <a id="nextPage" href='<%=basePath %>PageFile_queryPf.action?pageFile.currentPage=${pageFile.currentPage>=pageFile.totalPage?pageFile.currentPage:pageFile.currentPage+1}' >下一页</a>
           <a id="lastPage" href='<%=basePath %>PageFile_queryPf.action?pageFile.currentPage=${pageFile.totalPage}' >尾页</a>
		  &nbsp;&nbsp;&nbsp;共<span class='current'>${pageFile.count}</span>条记录<span class='current'> ${pageFile.count==0?"0":pageFile.currentPage }/${pageFile.totalPage} </span>页
           ...
           <div><button type="button" class="btn btn-primary" id="newPf">新增稿件</button></a>
         <button type="button" class="btn btn-primary" id="delPf" onclick="delAll();">删除稿件</button>
         ...
         <span  style=" color: #CCCCCY;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;tip:已绑定约稿的稿件不能删除或修改。</span>
		 </th></tr>
		 </div>
  </table>
