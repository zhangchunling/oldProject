...
<head>
    ...
    <script type="text/javascript">
		...
		$(function () {
		$('#goPageFile').click(function(){
				window.location.href="PageFile_queryPf.action?setNull=myManueGoPageFile"	;	
		 });
    });
    ...
    function exportExcel(){
			...
			if(ids.length>0){
				if(confirm("确认导出选中的约稿?")){
					window.location.href="MyManu_exportExcl.action?acceptState=subed&exportExcelType="+ids;
				}
			}else{
				if(confirm("请勾选要导出的稿件,[确定] 将导出所有约稿！")){
					window.location.href="MyManu_exportExcl.action?acceptState=subed&exportExcelType=all";
				}
			}
	 ...
	 $(document).ready(function(){
    	...
    	var h="MyManu_queryManu.action?acceptState=subed&flag=heh&mu.currentPage=";
    	var str="";
...
<table class="table table-bordered table-hover definewidth m10">
    <tr>
        ...
        <td><input type="text" id="mu_mname" name="mu.mname" value="${mu.mname }"/></td>
        <td width="10%" class="tableleft">刊物编号：</td>
        <td><input type="text" id="mu_mbknum" name="mu.mbknum" value="${mu.mbknum }"/></td>
    </tr>
    <tr>
        <td width="10%" class="tableleft">约稿发布时间：</td>
        <td><input type="text" id="mu_mstartTime" name="mu.mstartTime" value="${mu.mstartTime }"/></td>

        <td width="10%" class="tableleft">约稿截止时间：</td>
        <td><input type="text" id="mu_mendTime" name="mu.mendTime" value="${mu.mendTime }"/></td>
    </tr>
    <tr>	<input id="acceptState" name="acceptState" type="hidden" value="subed" />
		  <td colspan="6">
		  <center>
            <button type="submit" class="btn btn-primary" type="button">查询</button> 
            <button type="button" class="btn btn-primary" id="claer" type="button">清空</button> 
			</center>
        </td>
    </tr>
</table>
...
<table class="table table-bordered table-hover definewidth m10" >
  ...
  <c:forEach  var="m" items="${mus}" varStatus="status">
	     <tr >
            ...
            <td style="vertical-align:middle;"><a href="<%=basePath%>MyManu_queryManuById.action?mu.mid=${m.mid}">查看详情</a></td>
            <td>${m.umstate}</td>
  ...
  <table class="table table-bordered table-hover definewidth m10" >
  	<tr><th colspan="5">  <div class="inline pull-right page">
          <a id="firstPage" href='<%=basePath %>MyManu_queryManu.action?acceptState=subed&flag=heh&mu.currentPage=1' >首页</a> 
          <a id="upPage" href='<%=basePath %>MyManu_queryManu.action?acceptState=subed&flag=heh&mu.currentPage=${mu.currentPage<=1?mu.currentPage:mu.currentPage-1}'>上一页</a>    
       	 <span id="atag">
       	 </span> 
           ...
           <a id="nextPage" href='<%=basePath %>MyManu_queryManu.action?acceptState=subed&flag=heh&mu.currentPage=${mu.currentPage>=mu.totalPage?mu.currentPage:mu.currentPage+1}' >下一页</a>
           <a id="lastPage" href='<%=basePath %>MyManu_queryManu.action?acceptState=subed&flag=heh&mu.currentPage=${mu.totalPage}' >尾页</a>
		 ...
		 <button type="button" class="btn btn-primary" onclick="exportExcel();">导出Excel</button>
          <button type="button" class="btn btn-primary" id="goPageFile">进入我的草稿箱</button>
