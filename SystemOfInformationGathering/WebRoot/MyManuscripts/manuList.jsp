    ...
    function exportExcel(){
			...
			if(ids.length>0){
				if(confirm("确认导出选中的稿件信息?")){
					window.location.href="MyManu_exportExcl.action?acceptState=notAccept&exportExcelType="+ids;
				}
			}else{
				if(confirm("请勾选要导出的稿件,[确定] 将导出所有稿件信息！")){
					window.location.href="MyManu_exportExcl.action?acceptState=notAccept&exportExcelType=all";
				}
			}
	...
	function batAccept(){
			...
			if(ids.length>0){
				if(confirm("确认接受选中的约稿?")){
					window.location.href="MyManu_updateState.action?muuId="+ids;
				}
			}else{
				alert("请选择要接受的约稿");
			}
	 ...
	 $(document).ready(function(){
		if($('#listSize').val()==null){
		 ...
		 $("#msg").html($('#ManuAcceptMsg').val()).show(300).delay(3000).hide(300); //信息提示效果
    	var atag=$('#atag');
    	var totalPage=$('#totalPage').val();
    	var currentPage=$('#PageDomCtrl').val();
    	var h="MyManu_queryManu.action?acceptState=notAccept&flag=heh&mu.currentPage=";
    	var str="";
    ...
    //如果当前页=1则隐藏首页与上一页
     ...
     $(function () {
		$('#claer').click(function(){
				var wq1=$('#mu_mname').val("");
				var wq2=$('#mu_mbknum').val("");
				var wq3=$('#mu_mstartTime').val("");
				var wq4=$('#mu_mendTime').val("");
				var wq5=$('#mu_mmaster').val("");
...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
        ...
        <td><input type="text" id="mu_mmaster" name="mu.mmaster" value="${mymanu.mmaster }"/></td>
    </tr>
    <tr>
		  <td colspan="6">
		  <input id="acceptState" name="acceptState" type="hidden" value="notAccept">
			<center>
            <button type="submit" class="btn btn-primary" >查询</button> 
            <button type="button" class="btn btn-primary" id="claer">清空</button> 
			</center>
        </td>
    </tr>
</table>
...
<table class="table table-bordered table-hover definewidth m10" >
   <thead>
    ...
    <tr>
    	...
    	<tr>
            ...
            <td style="vertical-align:middle;">未接受</td>
    		<td style="vertical-align:middle;"><a href="<%=basePath%>MyManu_queryManuById.action?mu.mid=${m.mid}">查看详情</a>&nbsp;&nbsp;&nbsp;
    		<a href="<%=basePath%>MyManu_updateState.action?muu.mid=${m.mid}">接受</a>|<a href="<%=basePath%>MyManu_refuse.action?muu.mid=${m.mid}">拒绝</a>
  ...
  <table class="table table-bordered table-hover definewidth m10" >
  	<tr><th colspan="5">  <div class="inline pull-right page">
          <a id="firstPage" href='<%=basePath %>MyManu_queryManu.action?mu.currentPage=1&acceptState=notAccept&flag=heh' >首页</a> 
          <a id="upPage" href='<%=basePath %>MyManu_queryManu.action?mu.currentPage=${mu.currentPage<=1?mu.currentPage:mu.currentPage-1}&acceptState=notAccept&flag=heh'>上一页</a>    
       	 <span id="atag">
       	 </span> 
           ...
           <a id="nextPage" href='<%=basePath %>MyManu_queryManu.action?mu.currentPage=${mu.currentPage>=mu.totalPage?mu.currentPage:mu.currentPage+1}&acceptState=notAccept&flag=heh' >下一页</a>
           <a id="lastPage" href='<%=basePath %>MyManu_queryManu.action?mu.currentPage=${mu.totalPage}&acceptState=notAccept&flag=heh' >尾页</a>
         ...
         <button type="button" class="btn btn-primary" id="accepts" onclick="batAccept();">批量接受</button>
		 </div>
  ...
  <input type="hidden" value="${ManuAcceptMsg }" id="ManuAcceptMsg"/>
</body>
</html>
