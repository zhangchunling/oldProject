	...
	function roleBackJs() { 
		//回写性别
		var hiddenSex=parseInt(document.getElementById("sexBack").value);		
		var sexs=document.getElementsByName("employee.sex");
		for(var i=0;i<sexs.length;i++){
			if(sexs[i].value==hiddenSex){				
				sexs[i].checked=true;
			} 
		} 
		//回写角色
		 var roleArray=document.getElementsByName("strs");
		var roleBack=document.getElementById("roleBack").value;
		var roleBackArray=roleBack.split(",");
		for(var i=0;i<roleArray.length;i++){			
		
			for(var j=0;j<roleBackArray.length;j++){
				if(roleArray[i].value==parseInt(roleBackArray[j])){//字符串的数据字要解析下				
		...
		var stateArray = document.getElementsByName("employee.e_state");
   ...
   function checkName(){
   		...
   		var userId=document.getElementById("ceid").value
   		if(""==username.trim()){
			...
			b.className="yellow";			
   			...
   			//异步判断用户是否存在			
   			 ...
   			 $.ajax({   //后面跟一个时间的参数是为了防止有时候不提交的问题【冲破缓存，也可跟个随机数】	
   				 url:"employeePackage/employee_checkEditAjax.action?time="+today.getTime(),//访问路径
   				 type:"post",//提交方式
   				 data:"employeeName="+username, //username是action类里的属性
